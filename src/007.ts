...
const schema = z.object({
    ...
    data =>({
        error: `Para o nível ${data.nivel}, a experiência mínima é de 5 anos e é necessário ter pelo menos 2 tecnologias`,
        path: ["tecnologias"]
    })
    ).refine(
        data => data.nivel !== 'pleno' || (data.experienciaEmAnos >= 2 && data.tecnologias.length >= 1),
        data => ({
            error: `Para o nível ${data.nivel}, a experiência mínima é de 2 anos e é necessário ter pelo menos 1 tecnologia`,
        path: ["tecnologias"]
    }))
    ;
